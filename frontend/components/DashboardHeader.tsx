...
export function DashboardHeader({ user, onLogout, onMenuClick }: DashboardHeaderProps) {
    ...
    const headerClasses = `fixed top-4 h-16 bg-white rounded-md shadow-sm border border-[#E5E5E5] z-40 px-6 flex items-center justify-between transition-all duration-300 ${isBrand ? 'md:left-[252px]' : 'md:left-[272px]'} left-4 right-4`;
    ...
    return (
        <header className={headerClasses}>
            ...
            <div className="flex items-center gap-4">
                <button
                    className="md:hidden w-10 h-10 flex items-center justify-center text-[#6B6B6B] hover:bg-[#F4EFE6] hover:text-black rounded-md transition-colors"
                    onClick={onMenuClick}
                >
            ...
            <div className="flex items-center gap-4">
                <button
                    className="w-10 h-10 rounded-md bg-[#F4EFE6] flex items-center justify-center text-[#6B6B6B] hover:bg-[#FF4D00] hover:text-black hover:shadow-sm transition-all"
                ...
                <div className="relative" ref={notificationsRef}>
                    <button
                        className="w-10 h-10 rounded-md bg-[#F4EFE6] flex items-center justify-center text-[#6B6B6B] hover:bg-[#FF4D00] hover:text-black hover:shadow-sm transition-all relative"
                    ...
                    {notificationsOpen && (
                        <div className="absolute top-14 right-0 w-90 bg-white border border-[#E5E5E5] rounded-md shadow-strong z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                ...
                                {notifications.length > 0 ? (
                                    notifications.map(notif => (
                                        <div key={notif.id} className={`p-4 border-b border-[#E5E5E5] flex gap-3 hover:bg-[#F4EFE6] transition-colors cursor-pointer ${!notif.read ? 'bg-[#FFF7ED] border-l-[3px] border-l-[#FF4D00]' : 'opacity-70'}`}>
                                            <div className="w-9 h-9 rounded-sm bg-[#F4EFE6] flex items-center justify-center shrink-0">
                                                {notif.icon}
                                            </div>
                                            <div>
                ...
                <div className="relative" ref={profileRef}>
                    ...
                    {profileOpen && (
                        <div className="absolute top-14 right-0 w-70 bg-white border border-[#E5E5E5] rounded-md shadow-strong z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                            <div className="p-5 border-b border-[#E5E5E5] flex items-center gap-3">
                                ...
                                <div className="overflow-hidden">
                                    ...
                                    <div className="mt-1 inline-block px-2.5 py-1 rounded-sm bg-[rgba(255,61,0,0.15)]">
                    ...
                    <div className="search-modal" onClick={e => e.stopPropagation()}>
                        <div className="relative">
                            <input
                                ...
                                className="w-full h-13 bg-white border border-[#E5E5E5] rounded-md pl-5 pr-12 text-black font-sf-pro text-[16px] focus:outline-none focus:border-[#FF4D00] transition-colors placeholder:text-[#6B6B6B] lowercase"
                                value={searchQuery}
                        ...
                        {searchResults.length > 0 && (
                            <div className="search-results mt-4 max-h-100 overflow-y-auto flex flex-col gap-2">
                                {searchResults.map(result => (
                                    <div key={result.id} className="flex items-center gap-3 p-3 bg-white border border-[#E5E5E5] rounded-md hover:border-[#FF4D00] hover:bg-[#F9FAFB] hover:translate-x-1 transition-all cursor-pointer">
                                        ...
                                        <span className="text-[10px] font-sans font-bold text-black tracking-wider px-2 py-1 bg-[rgba(255,61,0,0.1)] rounded-sm">{result.type}</span>
                                    </div>
                                ))}
                            </div>
                        )}
