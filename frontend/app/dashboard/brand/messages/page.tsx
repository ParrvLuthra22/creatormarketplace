                    ...
                    <div className={`flex-1 flex flex-col ${!showMobileChat ? 'hidden md:flex' : 'flex'}`}>
                        {selectedConversation && chatPartner ? (
                            <>
                                ...
                                <div className="h-20 border-b border-zinc-100 flex items-center justify-between px-8 bg-white">
                                    ...
                                    <div className="relative">
                                        <button
                                            ...
                                            className="w-10 h-10 rounded-md bg-zinc-50 border border-zinc-200 flex items-center justify-center text-zinc-600 hover:bg-zinc-100 transition-all"
                                        >
                                            <MoreVertical className="w-5 h-5" />
                                        </button>

                                        {showMenu && (
                                            <div className="absolute right-0 top-full mt-2 w-52 bg-white border border-zinc-200 rounded-md p-2 z-10 shadow-xl overflow-hidden">
                                                <Link href={`/dashboard/brand/creators/${chatPartner._id}`} className="block w-full text-left px-4 py-2.5 text-sm text-zinc-700 hover:bg-zinc-50 rounded-md transition-all">
                                                    View Profile
                                                </Link>
                                                <button
                                                    onClick={handleCloseConversation}
                                                    className="w-full text-left px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 rounded-md transition-all flex items-center gap-2"
                                                >
                                ...
                                {/* Messages Area */}
                                <div id="messages-container" className="flex-1 overflow-y-auto px-8 py-8 space-y-4 scrollbar-hide bg-zinc-50">
                                    {messages.map((msg) => {
                                                ...
                                                {isMe && !msg.deleted && (
                                                    <div className={`absolute ${isMe ? '-left-20' : '-right-20'} top-1/2 -translate-y-1/2 hidden group-hover:flex items-center gap-1.5`}>
                                                        <button
                                                            ...
                                                            className="w-8 h-8 rounded-md bg-white border border-zinc-200 shadow-sm flex items-center justify-center hover:bg-zinc-50 transition-all"
                                                        >
                                                            <Pencil className="w-3.5 h-3.5 text-zinc-600" />
                                                        </button>
                                                        <button
                                                            onClick={(e) => { e.stopPropagation(); handleDeleteMessage(msg._id); }}
                                                            className="w-8 h-8 rounded-md bg-white border border-zinc-200 shadow-sm flex items-center justify-center hover:bg-red-50 transition-all"
                                                        >
