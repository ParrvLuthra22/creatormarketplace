...
// GET /api/chat/online-status?userIds=id1,id2,id3
router.get('/online-status', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const users = await User.find({ _id: { $in: userIds } }).select('_id lastSeen').lean();
...
// GET /api/chat/unread-count - Total and per-conversation unread count
router.get('/unread-count', authMiddleware, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const perConversation = breakdown.map(item => ({
            conversationId: item._id.toString(),
            unreadCount: item.count,
        }));

        const totalUnread = perConversation.reduce((total, item) => total + item.unreadCount, 0);
